...
frappe.ui.form.on("PD Peta Zona Pemenangan Sumatera Utara", {
  ...
  initialize_map: function (mapContainerId, frmInstance) {
      ...
      const backButton = L.Control.extend({
        ...
        onAdd: function () {
          let container = L.DomUtil.create(
            "div",
            "leaflet-bar leaflet-control leaflet-control-custom back-button sumut"
          );
    ...
    function fetchTableData(level, region) {
      const url = `polmarkdashboard.api.geojson.get_tabular_data?region=Sumatera Utara&region_level=${mapRenderLevel}&region_code=${region}`;
