...
// Global variable
let infoBoxTooltipId = "info-box-kota-medan";

frappe.ui.form.on("PD Peta Zona Pemenangan Kota Medan", {
  refresh(frm) {
  ...
  initialize_map: function (mapContainerId, frmInstance) {
      ...
      const backButton = L.Control.extend({
        ...
        onAdd: function () {
          let container = L.DomUtil.create(
            "div",
            "leaflet-bar leaflet-control leaflet-control-custom back-button kota-medan"
          );
    ...
    function loadMap(region_code, currentLevel, parentLevel, renderLevel) {
      ...
      const url = `polmarkdashboard.api.geojson.get_geojson_data_by_region?region=Kota Medan&region_level=${mapRenderLevel}&region_code=${region_code}`;
    ...
    function fetchTableData(level, region) {
      const url = `polmarkdashboard.api.geojson.get_tabular_data?region=Kota Medan&region_level=${mapRenderLevel}&region_code=${region}`;
